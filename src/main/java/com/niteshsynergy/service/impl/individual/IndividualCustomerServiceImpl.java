...
import static com.niteshsynergy.utils.individual.PrepareIndividualCustomersData.prepareIndividualCustomersData;
...
public class IndividualCustomerServiceImpl implements IndividualCustomerService {
    ...
    Logger log = LoggerFactory.getLogger(IndividualCustomerServiceImpl.class);
    ...
    public void deleteCustomerById(String customerId) {
        ...
        IndividualCustomers customer = mongoTemplate.findOne(query, IndividualCustomers.class);
    ...
    @Override
    public Optional<IndividualCustomers> findCustomerByPhone(String phone) {
        Query query = new Query(Criteria.where("phone").is(phone));
        return Optional.ofNullable(mongoTemplate.findById(query, IndividualCustomers.class));
    }
    ...
    public CompletableFuture<IndividualCustomers> saveIndividualCustomer(IndividualCustomers customer) {
        ...
        IndividualCustomers preparedCustomer = prepareIndividualCustomersData(customer);

        return CompletableFuture.supplyAsync(() -> {
            try {
                IndividualCustomers existingCustomer = mongoTemplate.findOne(
                        new Query(Criteria.where("phone").is(preparedCustomer.getPhone())),
                        IndividualCustomers.class
                );

                return (existingCustomer != null) ? existingCustomer : mongoTemplate.save(preparedCustomer);
            } catch (Exception e) {
                log.error("Error saving customer: {}, TraceId: {}, ParentId: {}", e.getMessage(), customer.getTraceId(), customer.getParentId());
                throw new CustomerSaveException(customer.getTraceId(), customer.getParentId(), e.getMessage());
            }
        }).thenApply(savedCustomer -> {
            if (savedCustomer == null) {
                throw new CustomerSaveException(customer.getTraceId(), customer.getParentId(), "Failed to save or fetch customer.");
            }
    ...
    public IndividualCustomers findCustomerById(String customerId) {
        return mongoTemplate.findById(customerId, IndividualCustomers.class);
    }

    private IndividualCustomerPayments saveFinalTransaction(IndividualCustomers customer, PaymentResponse paymentResponse) {
        if (customer.getId() == null) {
            log.error("Customer ID is null. Cannot save payment.");
            throw new IllegalStateException("Customer ID is null.");
        }

        IndividualCustomerPayments payment = IndividualCustomerPayments.builder()
        ...
        IndividualCustomerPayments savedPayment = mongoTemplate.save(payment);
    ...
    private void updateParentAmount(String parentId, Double childAmount) {
        IndividualCustomers parentCustomer = mongoTemplate.findById(parentId, IndividualCustomers.class);
    ...
    public void handleCustomerSaveFailure(String customerId) {
        System.out.println("Failed to save customer with ID: " + customerId + ". Please retry.");
    }
}
